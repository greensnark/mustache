...
func TestFSLookup(t *testing.T) {
	tests := []struct {
		...
		{"Simple partial lookup",
			"Test {{>embeddedPartial}} End",
			map[string]string{
				"embeddedPartial": "Embedded Partial",
			},
			"Test Embedded Partial End",
		},
	}
	...
	for _, test := range tests {
		...
		tmpl, err := ParseStringFS(test.template, "", fs)
		if err != nil {
			t.Errorf("Template parse error for %s: %s", test.name, err)
			continue
		}
		result := tmpl.Render()
		if result != test.expected {
			t.Errorf("Template result for %s == %#v, want %#v", test.name, result, test.expected)
		}

		result = RenderStringFS(test.template, fs)
		if result != test.expected {
			t.Errorf("%s: RenderStringFS(%#v, fs) == %#v, want %#v", test.name, result, test.expected)
		}
	}
}
